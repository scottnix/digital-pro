...
function digital_front_page_genesis_meta() {
	...
	if ( is_active_sidebar( 'front-page-1' ) || is_active_sidebar( 'front-page-2' ) || is_active_sidebar( 'front-page-3' ) ) {
		...
		function digital_enqueue_digital_script() {
			...
			// scott testing - remove since no longer loading this externally, merged with CSS. 
...
function digital_front_page_widgets() {
	...
	// scott - eh? could be something besides main content, removed for SEO reason, but needs something better than main content to warrant it.
	...
	// scott todo - removed front page section, this just pulls it out of content, need to remove from admin (functions.php)
